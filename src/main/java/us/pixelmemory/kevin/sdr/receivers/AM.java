...
public final class AM <T extends Throwable> implements IQSampleConsumer<T> {
	...
	public AM(double radioSampleRate, double radioTuningOffsetFrequency, double radioAFTRange, FloatConsumer<T> audioOutput, double audioSampleRate, float bandwidth, boolean debug) {
		...
		final double pipelineSampleRate = Math.max(audioSampleRate, bandwidth*2);
		...
		if (pipelineSampleRate > bandwidth*2) {
		...
		if (radioSampleRate > pipelineSampleRate) {
			...
			sink= new DownsamplerIQ<>(LanczosTable.of(6), radioSampleRate, pipelineSampleRate, new IQSampleBufferThread<>(2048, "AM tuner radio downsample " + radioSampleRate + " to " + pipelineSampleRate, sink));
			if (debug && enableDebug) {
		...
		final boolean needDownsample= audioSampleRate < pipelineSampleRate;
		
		audioHighPassFilter= new RCHighPass(pipelineSampleRate, carrierAGCStrengthRc); //DC block, just in case the signal is distorted
		audioSink= needDownsample ? new Downsampler<>(LanczosTable.of(6), pipelineSampleRate, audioSampleRate, audioOutput) : audioOutput;
		synchronousTuner= new PhaseLock(pipelineSampleRate, 0.1, radioAFTRange, debug && enableDebug);
		...
		if (debug && enableDebug) {
			...
			System.out.println("needDownsample=" + needDownsample);
		}
	}
	...
	private void processSample (IQSample iq) throws T {
		...
		float tunedSignal= (float)synchronousTunedSample.in;
		...
		if (vis != null) {
			...
			vis.drawAnalog(Color.pink, 10*synchronousStrength);
